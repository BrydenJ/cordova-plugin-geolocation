...
This plugin defines a global `pluginGeolocation` object (for platforms
...
- __String__: returns a watch id that references the watch position interval. The watch id should be used with `pluginGeolocation.clearWatch` to stop watching for changes in position.
...
callback function when an error occurs with pluginGeolocation.
...
- `PositionError.TIMEOUT`
  - Returned when the device is unable to retrieve a position within the time specified by the `timeout` included in `geolocationOptions`. When used with `pluginGeolocation.watchPosition`, this error could be repeatedly passed to the `geolocationError` callback every `timeout` milliseconds.
...
function watchWeatherPosition() {
    ...
    return pluginGeolocation.watchPosition
